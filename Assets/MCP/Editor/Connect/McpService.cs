    ...
    public partial class McpService
    {
        ...
        public static string GetPortStatusInfo(int port)
        {
            try
            {
                ...
                info.AppendLine($"Port {port} Status Check:");

                // Check if port is in use
                bool inUse = Instance.IsPortInUse(port);
                info.AppendLine($"- Port Status: {(inUse ? "In Use" : "Available")}");
                ...
                try
                {
                    ...
                    {
                        info.AppendLine($"- Netstat Output:");
                        info.AppendLine(output);
                    }
                    else
                    {
                        info.AppendLine($"- Netstat: Port {port} not in use");
                    }
                }
                catch (Exception ex)
                {
                    info.AppendLine($"- Netstat check failed: {ex.Message}");
                }
            ...
            {
                return $"Port status check failed: {ex.Message}";
            }
        }
