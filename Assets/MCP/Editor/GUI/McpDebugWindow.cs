...
namespace UniMcp.Gui
{
    ...
    public class McpDebugWindow : EditorWindow
    {
        ...
        private JsonNode currentResult = null; // 存储当前Result
        ...
        private void InitializeStyles()
        {
            ...
            if (codeStyle == null)
            {
                codeStyle = new GUIStyle(EditorStyles.textArea)
                {
                    ...
                    stretchHeight = true   // 拉伸以适应容器Height
                };
            }
            ...
            if (inputStyle == null)
            {
                inputStyle = new GUIStyle(EditorStyles.textArea)
                {
                    ...
                    stretchHeight = true,   // 允许Height拉伸
                    fixedWidth = 0,         // 不使用固定宽度
                    fixedHeight = 0,        // 不使用固定Height
        ...
        private void InitializeRecordList()
        {
            if (recordList == null)
            {
                ...
                recordList.drawHeaderCallback = (Rect rect) =>
                {
                    ...
                    if (groups.Count == 0)
                    {
                        EditorGUI.LabelField(rect, "No groups");
                    }
                    else
                    {
                        // 构建分组选项（包含统计信息）
                ...
                recordList.onRemoveCallback = (ReorderableList list) =>
                {
                    ...
                    if (list.index >= 0 && list.index < records.Count)
                    {
                        ...
                        if (EditorUtility.DisplayDialog("Confirm Delete", $"Delete这条执行记录吗？\n函数: {records[actualIndex].name}", "删除", "Cancel"))
                        {
                ...
                recordList.elementHeight = 40f; // 设置元素Height
            }
        }

        /// <summary>
        /// 根据文本内容动态计算输入框Height（考虑自动换行和固定宽度）
        /// </summary>
        ...
        {
            // 标题文字Height（基于headerStyle的fontSize）
        ...
        private void DrawConnectionStatus(Rect rect)
        {
            ...
            string fullStatusText = $"<color=#{ColorUtility.ToHtmlStringRGB(statusColor)}>●</color> <b>{statusText}</b> ({clientCount} requests{portText})";

            // Draw a single button that covers the whole area and acts as the status display
            if (GUI.Button(rect, new GUIContent(fullStatusText, "点击查看requests详情"), statusButtonStyle))
            {
        ...
        private void DrawLeftPanel(Rect rect)
        {
            ...
            if (GUILayout.Button("Clear Group", GUILayout.Width(100)))
            {
                string confirmMessage = $"Clear all records in group '{GetCurrentGroupDisplayName()}' ?\nThis will not affect other groups.";

                if (EditorUtility.DisplayDialog("Confirm Clear", confirmMessage, "OK", "Cancel"))
                {
                    // 仅清空当前分组，禁止清空全部
            ...
            if (GUILayout.Button(showGroupManager ? "Hide" : "Groups", GUILayout.Width(60)))
            {
        ...
        private void DrawInputArea(float availableWidth)
        {
            ...
            GUILayout.Label($"Lines: {lineCount} | Height: {inputHeight:F0}px", EditorStyles.miniLabel);
        }
        ...
        private void DrawControlButtons()
        {
            ...
            if (GUILayout.Button("Run Clipboard", GUILayout.Height(30), GUILayout.Width(100)))
            {
                ExecuteClipboard();
            }
            GUI.enabled = true;

            if (GUILayout.Button("Format JSON", GUILayout.Height(30), GUILayout.Width(120)))
            {
                FormatJson();
            }
            ...
            if (GUILayout.Button("Paste to Input", GUILayout.Height(25), GUILayout.Width(100)))
            {
                PasteFromClipboard();
            }
        ...
        private void DrawResultArea(float availableWidth)
        {
            ...
            // Record按钮 - 只有当有Result且不是从历史记录加载时才显示
            if (currentResult != null && !string.IsNullOrEmpty(inputJson))
            {
                if (GUILayout.Button("Record", GUILayout.Width(80)))
                {
                    RecordCurrentResult();
                }
            }

            // Format按钮 - 只有当有结果文本时才显示
            ...
            {
                if (GUILayout.Button("Copy Stats", GUILayout.Width(80)))
                {
                    CopyBatchStatistics();
                }

                if (GUILayout.Button("Errors Only", GUILayout.Width(80)))
                {
                    ShowOnlyErrors();
                }
            }
        ...
        private void FormatJson()
        {
            ...
            {
                EditorUtility.DisplayDialog("JSON Error", $"Cannot parse JSON: {e.Message}", "OK");
            }
        }

        /// <summary>
        /// Format文本，支持JSON和YAML
        /// </summary>
        private void FormatResultText()
        {
            if (string.IsNullOrEmpty(resultText))
            {
                EditorUtility.DisplayDialog("Info", "No result to format", "OK");
                return;
            }
            ...
            {
                Debug.LogWarning($"[McpDebugWindow] Format时发生错误: {e.Message}");
                EditorUtility.DisplayDialog("格式化失败", $"无法Format: {e.Message}", "OK");
            }
        }
        ...
        private void ExecuteCall()
        {
            ...
            {
                EditorUtility.DisplayDialog("错误", "请输入JSON内容", "OK");
                return;
            }
        ...
        private JsonNode ExecuteJsonCallInternal(string jsonString, DateTime startTime, System.Action<JsonNode, TimeSpan> onSingleComplete)
        {
            ...
            if (inputObj == null)
                return Response.Error("Cannot parse JSON输入");
        ...
        private void CopyBatchStatistics()
        {
            ...
            try
            {
                ...
                EditorUtility.DisplayDialog("Copied", "Statistics copied to clipboard", "OK");
            }
            catch (Exception e)
            {
                EditorUtility.DisplayDialog("Copy Failed", $"Cannot copy stats: {e.Message}", "OK");
            }
        }
        ...
        private void ShowOnlyErrors()
        {
            ...
            try
            {
                ...
                output.AppendLine($"Failed Calls: {failedCalls}");
                output.AppendLine();
            ...
            {
                resultText = $"Failed to show errors: {e.Message}";
            }
        }

        /// <summary>
        /// Run Clipboard中的JSON内容
        /// </summary>
        private void ExecuteClipboard()
        {
            try
            {
                ...
                {
                    EditorUtility.DisplayDialog("JSON Error", $"Clipboard content is not valid JSON:\n{errorMessage}", "OK");
                    return;
                }
                ...
                isExecuting = true;
                showResult = true;
                resultText = "Running clipboard content...";
                ...
                try
                {
                    ...
                    if (result == null)
                    {
                        resultText = "Running clipboard async...";
                        // 刷新界面显示异步状态
                        Repaint();
                        // 注意：isExecuting保持为true，等待异步回调完成
                    }
                    else
                    {
                        ...
                        currentResult = result;
                        string formattedResult = FormatResult(result, duration);
                        resultText = $"📋 Executed from clipboard\nOriginal JSON:\n{clipboardContent}\n\n{formattedResult}";
                ...
                {
                    string errorResult = $"Clipboard execution error:\n{e.Message}\n\nStack trace:\n{e.StackTrace}";
                    resultText = errorResult;

                    Debug.LogError($"[McpDebugWindow] Clipboard execution error: {e}");
                }
                finally
                {
                    isExecuting = false;
            ...
            {
                EditorUtility.DisplayDialog("Execution Failed", $"Cannot run clipboard: {e.Message}", "OK");
                isExecuting = false;
            }
        }
        ...
        private void PasteFromClipboard()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(clipboardContent))
                {
                    EditorUtility.DisplayDialog("Info", "Clipboard is empty", "OK");
                    return;
                }
                ...
                if (!ValidateClipboardJson(clipboardContent, out string errorMessage))
                {
                    bool proceed = EditorUtility.DisplayDialog("JSON Format Warning",
                        $"Clipboard may not contain valid JSON:\n{errorMessage}\n\nPaste anyway?",
                        "Paste", "Cancel");
                ...
                EditorUtility.DisplayDialog("Success", "Pasted clipboard content to input", "OK");
            }
            catch (Exception e)
            {
                EditorUtility.DisplayDialog("Paste Failed", $"Cannot paste clipboard: {e.Message}", "OK");
            }
        }
        ...
        private void PreviewClipboard()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(clipboardContent))
                {
                    EditorUtility.DisplayDialog("Clipboard Preview", "Clipboard is empty", "OK");
                    return;
                }
            ...
            {
                EditorUtility.DisplayDialog("Preview Failed", $"Cannot preview clipboard: {e.Message}", "OK");
            }
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (isValidJson)
                    {
                        ...
                        statusText = $"Clipboard: ✅ Json ({clipboardContent.Length} chars)";
                    }
                    else
                    {
                        statusColor = new Color(1f, 0.5f, 0f); // 橙色
                        statusText = $"Clipboard: ❌ Non-JSON ({clipboardContent.Length} chars)";
                    }
                }
        ...
        private void RecordCurrentResult()
        {
            if (currentResult == null || string.IsNullOrEmpty(inputJson))
            {
                EditorUtility.DisplayDialog("无法记录", "没有可记录的Result", "OK");
                return;
            }
            ...
            try
            {
                ...
                {
                    EditorUtility.DisplayDialog("记录失败", "无法解析输入的JSON格式", "OK");
                    return;
                }

                EditorUtility.DisplayDialog("记录成功", "Result已保存到记录中", "OK");
            ...
            {
                EditorUtility.DisplayDialog("记录失败", $"记录Result时发生错误: {e.Message}", "OK");
                Debug.LogError($"[McpDebugWindow] 手动Record时发生错误: {e}");
            }
        }
        ...
        private void DrawGroupManager(float width)
        {
            ...
            if (GUILayout.Button("创建分组", GUILayout.Width(80)))
            {
                ...
                if (recordObject.CreateGroup(groupId, groupNameTrimmed, newGroupDescription.Trim()))
                {
                    newGroupName = "";
                    newGroupDescription = "";
                    EditorUtility.DisplayDialog("成功", $"分组 '{groupNameTrimmed}' 创建成功！", "OK");
                }
                else
                {
                    EditorUtility.DisplayDialog("失败", "创建分组失败，请检查名称是否重复。", "OK");
                }
            }
            GUI.enabled = true;
            GUILayout.EndHorizontal();
            ...
            if (recordObject.recordGroups.Count > 0)
            {
                ...
                for (int i = 0; i < recordObject.recordGroups.Count; i++)
                {
                    ...
                    // 操作按钮（水平排列）
                    if (GUILayout.Button("Use", GUILayout.Width(50), GUILayout.Height(20)))
                    {
                    ...
                    if (GUILayout.Button("Del", GUILayout.Width(50), GUILayout.Height(20)))
                    {
                        if (EditorUtility.DisplayDialog("Confirm Delete",
                            $"Delete group '{group.name}'?\n\nAll records will be moved to Default Group.",
                            "Delete", "Cancel"))
                        {
        ...
        {
            var recordObject = McpExecuteRecordObject.instance;
            float baseHeight = 120; // 基本Height（标题 + 创建区域）

            if (recordObject.recordGroups.Count > 0)
            {
                baseHeight += 140; // 分组列表区域（标题 + 固定Height的滚动区域）
            }
